...
Class Equipo 
{
	...
	public function select(){
		$sql = "SELECT 	e.equi_id,
						e.equi_referencia,
						e.equi_tipo_id,
						e.equi_descripcion,
						e.equi_fabricante_id,
						e.equi_estado,
						f.fab_nombre
				FROM equipo e
				INNER JOIN fabricante f
				ON f.fab_id = e.equi_fabricante_id
				where equi_estado = 1";
	...
	public function stock($equi_det_equipo_id)
	{
		$sql = "SELECT count(equi_det_mac) FROM equipo_detalle 
				WHERE equi_det_equipo_id = '$equi_det_equipo_id' 
				AND equi_det_estado_id = 1";

		return ejecutarConsultaSimpleFila($sql);
	}

	public function instalado($equi_det_equipo_id)
	{
		$sql = "SELECT count(equi_det_mac) FROM equipo_detalle 
				WHERE equi_det_equipo_id = '$equi_det_equipo_id' 
				AND equi_det_estado_id = 2";

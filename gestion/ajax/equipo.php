...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		while ($reg = $respuesta->fetch_object()) {
			...
			$stock 			= 	$equipo->stock($reg->equi_id);
			$instalados		= 	$equipo->instalado($reg->equi_id);						

			$contarItems 	= 	implode('', $contarItems);
			$stock 			= 	implode('', $stock);
			$instalados		= 	implode('', $instalados);
			// print_r($contarItems);
			// $probar  	= 	$contarItems;
			...
			$data[] = array(
				...
				<button  title="Detalle" data-toggle="tooltip" title="Detalle" class="btn btn-info" onclick="detalle('.$reg->equi_id.')">
					<i class="fa fa-eye"></i>
				</button>'." ".	'':'

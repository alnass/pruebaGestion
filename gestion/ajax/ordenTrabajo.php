...
// LES VALEURS ENVOYÉES DEPUIS LE FORMULAIRE D'INSCRIPTION
...
// VERIFIEZ SI LA VALEUS DE L'OPERATION EST VIDE, DAN CETTE MANIERE, SI C'EST UN NOUVEAU CONTRAT
if(empty($_POST['ord_trab_operacion_id']))
{
	// ASSIGNER L'OPERATION LE NOMBRE 1 QUI EST L'ETAT A INSTALLER 
...
switch($_GET["op"])
{
	...
	require_once 	'../modelos/OrdenTrabajo.php';
		...
		$respuesta2	= 	$ot_activa->validarOTActiva($ord_trab_contrato_id);
			...
			else
			{
				...
				{
					//valider si un équipement a éte assigné dans l'ordre de travail 
					if(empty($_POST['id_detalle_equipo']))
					{
						$respuesta 	= 	$seguimientoOT->insertar(
									'null',
					...
					{
						// Stocker les données avec l'équipement attribué
	...
	case 'listar':
		...
		{
			$respuesta = $ordenTrabajo->listarFiltrado($_GET['filtrar']);	
		}
		...
		while ($reg = $respuesta->fetch_object()) {
			...
			elseif ($reg->cont_estado_servicio_id == 3) {
				$estadoservicio = '<span class="label bg-red">03 Por Cortar</span>';
			}
			...
			elseif ($reg->cont_estado_servicio_id == 7) {
				$estadoservicio = '<span class="label bg-orange">06 Reco - Susp</span>';
			}
			...
			elseif ($reg->cont_estado_servicio_id == 11) {
				$estadoservicio = '<span class="label bg-gray">07 Corte por retiro</span>';
			}
	...
	case 'listarEquipoInstalado':
		...
		case 'listarCantidadOts':

			$respuesta 	= 	$ordenTrabajo->listarServicios();

			$data 	= Array();

			while($reg = $respuesta->fetch_object())
			{
				if($reg->est_serv_id != 2 && $reg->est_serv_id != 4 && $reg->est_serv_id != 6 && $reg->est_serv_id != 10 && $reg->est_serv_id != 7 )
				{
					if ($reg->est_serv_id == 0) 
					{
						$servicio = 'Error ST1001';
					}
					elseif ($reg->est_serv_id == 1) 
					{
						$servicio = '<a href="#"><span class="label bg-yellow" onclick="listarFiltrado('.$reg->est_serv_id.')">01a Por instalar</span></a>';
					}
					elseif ($reg->est_serv_id == 3) 
					{
						$servicio = '<a href="#"><span class="label bg-red" onclick="listarFiltrado('.$reg->est_serv_id.')">02a Por Cortar</span></a>';
					}
					elseif ($reg->est_serv_id == 5) 
					{
						$servicio = '<a href="#"><span class="label bg-blue" onclick="listarFiltrado('.$reg->est_serv_id.')">02a Por reconectar</span></a>';
					}
					elseif ($reg->est_serv_id == 8)
					{
						$servicio = '<a href="#"><span class="label bg-purple" onclick="listarFiltrado('.$reg->est_serv_id.')">03b Por suspender</span></a>';
					}
					elseif ($reg->est_serv_id == 9)
					{
						$servicio = '<a href="#"><span class="label bg-gray" onclick="listarFiltrado('.$reg->est_serv_id.')">02b Mantenimiento</span></a>';
					}
					elseif ($reg->est_serv_id == 11) 
					{
						$servicio = '<a href="#"><span class="label bg-gray" onclick="listarFiltrado('.$reg->est_serv_id.')">03b Corte por retiro</span></a>';
					}
					elseif ($reg->est_serv_id == 12) 
					{
						$servicio = '<a href="#"><span class="label bg-red" onclick="listarFiltrado('.$reg->est_serv_id.')">02c Por traslado</span></a>';
					}
					elseif ($reg->est_serv_id == 13) 
					{
						$servicio = '<a href="#"><span class="label bg-red" onclick="listarFiltrado('.$reg->est_serv_id.')">01 Agr. Producto</span></a>';	
					}
					elseif($reg->est_serv_id >= 14)
					{
						$servicio 	= '<a href="#"><span class="label label-info" onclick="listarFiltrado('.$reg->est_serv_id.')">01b Camb. Product</span></a>';	
					}


					$cant_OTs 	= 	$ordenTrabajo->listarCantOtSede($reg->est_serv_id, $_SESSION['usu_sede_id']);
					...
					if($cant_OTs['total'] != 0)
					{
						...
						// Declaracion de indices de almacenamiento dentro del array
						"0"=>$cant_OTs['total'],
						"1"=>$cant_OTs['cont_estado_servicio_id'],
						"1"=>$servicio);
					}
				}
			}

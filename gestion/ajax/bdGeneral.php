<?php  
session_start();
require "../modelos/BdGeneral.php";

$bdGeneral = new BdGeneral();

switch ($_GET['op']) {
	case 'listar':
		$resultado = $bdGeneral->listar();

		$data = ArraY();

		while ($reg = $resultado->fetch_object()) {
			
			$saldos = $bdGeneral->saldos($reg->cont_id);
			$saldo_anterior = $saldos['est_cta_saldo_anterior'];
			$saldo_actual = $saldos['est_cta_saldo_actual'];
			...
			$t = 0;
				...
				{
					$t = ceil($saldo_actual/$reg->cont_valor_total_mes);// devuelve el valor redondeado en entero mayor 
				}
			...
			$data[] = array(
				...
				"8"=>'$'.number_format($saldo_anterior),
				"9"=>'$'.number_format($saldo_actual),
				"10"=>number_format($t),
				"11"=>$reg->sed_nombre,
				"12"=>$estadoservicio
			);

...
require "../modelos/consolidadoFacturas.php";
$consolidadoFacturas = new ConsolidadoFacturas();

switch ($_GET['op']) {
	
	case 'listar':
		$respuesta = $consolidadoFacturas->traerMes();

		$data = Array();

		while (	$reg = $respuesta->fetch_object()) {
			$mes  = "";
			$bogota = $consolidadoFacturas->conBogota($reg->fecha);
			$paipa = $consolidadoFacturas->conPaipa($reg->fecha);
			$fira = $consolidadoFacturas->conFira($reg->fecha);
			$tibasosa = $consolidadoFacturas->conTibasosa($reg->fecha);
			$iza = $consolidadoFacturas->conIza($reg->fecha);
			$fomeque = $consolidadoFacturas->conFomeque($reg->fecha);
			$snAntonio = $consolidadoFacturas->conSnAntonio($reg->fecha);
			$madrid = $consolidadoFacturas->conMadrid($reg->fecha);
			$corp = $consolidadoFacturas->conCorp($reg->fecha);
			$total = $consolidadoFacturas->total($reg->fecha);
			...
			$data[] = array(
				"0"=>$mes,
				"1"=>"$".number_format($bogota['sumatoria']),
				"2"=>"$".number_format($paipa['sumatoria']),
				"3"=>"$".number_format($fira['sumatoria']),
				"4"=>"$".number_format($tibasosa['sumatoria']),
				"5"=>"$".number_format($iza['sumatoria']),
				"6"=>"$".number_format($fomeque['sumatoria']),
				"7"=>"$".number_format($snAntonio['sumatoria']),
				"8"=>"$".number_format($corp['sumatoria']),
				"9"=>"$".number_format($madrid['sumatoria']),
				"10"=>"$".number_format($total['sumatoria'])
			);			
		}
		$results = array(
...
function reporte($valor)
{
	...
	while($reg = $respuesta->fetch_object())
	{
		...
		$est_cta 		= 	$consolidadoFacturas->estadoCuentaContrato($reg->fecha);
		$est_cta_corp 	= 	$consolidadoFacturas->estadoCuentaContratoCorp($reg->fecha);

		while ($est_cta_cli = $est_cta->fetch_object()) 
		{
			// BUSCA LOS PRODUCTOS DE TIPO TV QUE TENGA DEL CONTRATO
			$listado 		= 	$consolidadoFacturas->listarcontrato($est_cta_cli->est_cta_contrato_id);

			if($valor == 'tv')
			{
				$reg_prod		= 	$consolidadoFacturas->validarProductoTv($est_cta_cli->est_cta_contrato_id);
			}
			else if($valor == 'tvd')
			{
				$reg_prod		= 	$consolidadoFacturas->validarProductoTvDG($est_cta_cli->est_cta_contrato_id);
			}
			else if($valor == 'otros')
			{
				$reg_prod		= 	$consolidadoFacturas->validarProductoOtro($est_cta_cli->est_cta_contrato_id);
			}
			else
			{
				$reg_prod		= 	$consolidadoFacturas->validarProductoTv($est_cta_cli->est_cta_contrato_id);	
			...
			{// SI NO ESTA VACIO EL REGISTRO CREA UN ARRAY PARA CALCULAR EL PORCENTAJE EQUIVALENTE DE TV CON RESPECTO AL VALOR GENERAL DEL PRODUCTO
				...
				$pago 	=	array(	$valor 	=> 	$est_cta_cli->est_cta_haber * $porcentaje[$valor] / 100,
									'int' 	=> 	$est_cta_cli->est_cta_haber * $porcentaje['int'] / 100);
			...
			else
			{
				...
				$porcentaje['int'] 	=	100;
				$pago[$valor] 		= 	0;
				$pago['int'] 		= 	$est_cta_cli->est_cta_haber;
				$total[$valor] 		= 	$total[$valor] + $pago[$valor];
				$total['int'] 		= 	$total['int'] + $pago['int'];
				$contador++;
			}
			...
			// {
			// 	echo $contador. ' Mes '  . $reg->fecha. '<h1> Sede '. $est_cta_cli->est_cta_sede_id. ' Contrato '. $listado_und->cont_id . '</h1> Porcentaje Tv ' .$porcentaje['tv']. '% Porcentaje Internet '.$porcentaje['int'] .'% <br> Pagó '. $est_cta_cli->est_cta_haber .' Mensualidad ' .$listado_und->cont_valor_total_mes  .' Pago tv $'.round($pago['tv']).' Pago int $'. round($pago['int']).'<br>
			...
			// else
			// {
			// 	echo $contador. ' Mes '  . $reg->fecha. ' Sede '. $est_cta_cli->est_cta_sede_id. ' Contrato '. $listado_und->cont_id . ' Porcentaje Tv ' .$porcentaje['tv']. '% Porcentaje Internet '.$porcentaje['int'] .'% <br> Pagó '. $est_cta_cli->est_cta_haber .' Mensualidad ' .$listado_und->cont_valor_total_mes  .' Pago tv $'.round($pago['tv']).' Pago int $'. round($pago['int']).'<br>
			...
			// }
			// echo $contador. ' Mes ' . $reg->fecha. ' Contrato ' .$est_cta_cli->est_cta_contrato_id . ' Pagó '.$est_cta_cli->est_cta_haber.'<br>';
		}
		...
		{
			// BUSCA LOS PRODUCTOS DE TIPO TV QUE TENGA DEL CONTRATO
			$listado_corp 		= 	$consolidadoFacturas->listarcontrato($est_cta_cli_corp->est_cta_contrato_id);

			if($valor == 'tv')
			{
				$reg_prod_corp		= 	$consolidadoFacturas->validarProductoTv($est_cta_cli_corp->est_cta_contrato_id);
			}
			else if($valor == 'tvd')
			{
				$reg_prod_corp		= 	$consolidadoFacturas->validarProductoTvDG($est_cta_cli_corp->est_cta_contrato_id);
			}
			else if($valor == 'otros')
			{
				$reg_prod_corp		= 	$consolidadoFacturas->validarProductoOtro($est_cta_cli_corp->est_cta_contrato_id);
			}
			else
			{
				$reg_prod_corp		= 	$consolidadoFacturas->validarProductoTv($est_cta_cli_corp->est_cta_contrato_id);	
			...
			{// SI NO ESTA VACIO EL REGISTRO CREA UN ARRAY PARA CALCULAR EL PORCENTAJE EQUIVALENTE DE TV CON RESPECTO AL VALOR GENERAL DEL PRODUCTO
				...
				/// OPERA EL VALOR RECAUDADO CON RESPECTO AL PORCENTAJE OBTENIDO EN LAS OPERACIONES ANTERIORES
				$pago_corp 	=	array(	$valor 	=> 	$est_cta_cli_corp->est_cta_haber * $porcentaje_corp[$valor] / 100,
									'int' 	=> 	$est_cta_cli_corp->est_cta_haber * $porcentaje_corp['int'] / 100);
				// SUMA EL RESULTADO A LA POSICION DEL ARRAY GENERAL CORRESPONDIENTE AL PRODUCTO
			...
			else
			{
				...
				$pago_corp[$valor] 			= 	0;
				$pago_corp['int'] 			= 	$est_cta_cli_corp->est_cta_haber;
				$total[$valor] 				= 	$total[$valor] + $pago_corp[$valor];
				$total['int'] 				= 	$total['int'] + $pago_corp['int'];
				$contador_corp++;
			}

...
{
	$operacionesAdmin->control_impr_cuentas(1,$_SESSION['usu_id']);
	$respuesta 	=	$cobro->listarporSede($_SESSION['usu_sede_id']);	
}
else
{
	$respuesta 	=	$cobro->listarporSede($_REQUEST['sede']);
}

// $pdf= new PDF('L', 'mm', array(210, 140));// (ORIENTACION, MEDIDA, (ANCHO, ALTO))
$pdf= new PDF();// (ORIENTACION, MEDIDA, (ANCHO, ALTO))
$pdf->AliasNbPages();
...
while ($contador < $respuesta->num_rows)/// INDIVIDUAL
{
	$contador++;
	...
	$producto_clie 	= $call_producto->listarProducto($row['cont_id']);

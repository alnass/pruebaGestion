...
function listar(){
	...
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":{
			url:'../ajax/tipoVivienda.php?op=listar',
			type: "get",
			dataType: "json",
			error: function(e){
...
function guardaryeditar(e){
	...
	$.ajax({
		...
		url: "../ajax/tipoVivienda.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(tip_viv_id){
	$.post("../ajax/tipoVivienda.php?op=mostrar",{tip_viv_id : tip_viv_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#tip_viv_id").val(data.tip_viv_id);
		$("#nombre").val(data.tip_viv_nombre);
		$("#desc").val(data.tip_viv_descripcion);
	})
}

// Funcion para desactivar un registro
function desactivar(tip_viv_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de vivienda?",function(result){
		if (result) {
			$.post("../ajax/tipoVivienda.php?op=desactivar", {tip_viv_id : tip_viv_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(tip_viv_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de vivienda?",function(result){
		if (result) {
			$.post("../ajax/tipoVivienda.php?op=activar", {tip_viv_id : tip_viv_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}

...
function listar(){
	...
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":{
			url:'../ajax/tipoCanal.php?op=listar',
			type: "get",
			dataType: "json",
			error: function(e){
...
function guardaryeditar(e){
	...
	$.ajax({
		...
		url: "../ajax/tipoCanal.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(tip_can_id){
	$.post("../ajax/tipoCanal.php?op=mostrar",{tip_can_id : tip_can_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#tip_can_id").val(data.tip_can_id);
		$("#nombre").val(data.tip_can_nombre);
		$("#desc").val(data.tip_can_descripcion);
	})
}

// Funcion para desactivar un registro
function desactivar(tip_can_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/tipoCanal.php?op=desactivar", {tip_can_id : tip_can_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(tip_can_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/tipoCanal.php?op=activar", {tip_can_id : tip_can_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}

...
function mostrar(are_id){
	$.post("../ajax/area.php?op=mostrar",{are_id : are_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#are_id").val(data.are_id);
		$("#nombre").val(data.are_nombre);
		$("#desc").val(data.are_descripcion);
		$("#correo").val(data.are_correo);
	})
}

// Funcion para desactivar un registro
function desactivar(are_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/area.php?op=desactivar", {are_id : are_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(are_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/area.php?op=activar", {are_id : are_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
function bloq(){
	bootbox.alert("<strong>Esta área contiene permisos especiales y no puede ser modificada o desactivada</strong><br>Contacte con el administrador del sistema");
}

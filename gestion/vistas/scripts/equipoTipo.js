...
function listar(){
	...
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":{
			url:'../ajax/equipoTipo.php?op=listar',
			type: "get",
			dataType: "json",
			error: function(e){
...
function guardaryeditar(e){
	...
	$.ajax({
		...
		url: "../ajax/equipoTipo.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(equi_tip_id){
	$.post("../ajax/equipoTipo.php?op=mostrar",{equi_tip_id : equi_tip_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#equi_tip_id").val(data.equi_tip_id);
...
function desactivar(equi_tip_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de equipo?",function(result){
		if (result) {
			$.post("../ajax/equipoTipo.php?op=desactivar", {equi_tip_id : equi_tip_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(equi_tip_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de equipo?",function(result){
		if (result) {
			$.post("../ajax/equipoTipo.php?op=activar", {equi_tip_id : equi_tip_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}

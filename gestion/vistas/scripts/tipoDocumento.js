...
function mostrar(tip_doc_id){
	$.post("../ajax/tipoDocumento.php?op=mostrar",{tip_doc_id : tip_doc_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#tip_doc_id").val(data.tip_doc_id);
		$("#nombre").val(data.tip_doc_nombre);
		$("#desc").val(data.tip_doc_descripcion);
	})
}

// Funcion para desactivar un registro
function desactivar(tip_doc_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de documento?",function(result){
		if (result) {
			$.post("../ajax/tipoDocumento.php?op=desactivar", {tip_doc_id : tip_doc_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(tip_doc_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de documento?",function(result){
		if (result) {
			$.post("../ajax/tipoDocumento.php?op=activar", {tip_doc_id : tip_doc_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}

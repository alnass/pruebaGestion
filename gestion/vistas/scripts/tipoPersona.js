...
function mostrar(tip_per_id){
	$.post("../ajax/tipoPersona.php?op=mostrar",{tip_per_id : tip_per_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#tip_per_id").val(data.tip_per_id);
		$("#nombre").val(data.tip_per_nombre);
		$("#desc").val(data.tip_per_descripcion);
	})
}

// Funcion para desactivar un registro
function desactivar(tip_per_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/tipoPersona.php?op=desactivar", {tip_per_id : tip_per_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(tip_per_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/tipoPersona.php?op=activar", {tip_per_id : tip_per_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}

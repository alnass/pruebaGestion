...
function listar(){
	...
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":{
			url:'../ajax/estadoPqr.php?op=listar',
			type: "get",
			dataType: "json",
			error: function(e){
...
function guardaryeditar(e){
	...
	$.ajax({
		...
		url: "../ajax/estadoPqr.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(est_pqr_id){
	$.post("../ajax/estadoPqr.php?op=mostrar",{est_pqr_id : est_pqr_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#est_pqr_id").val(data.est_pqr_id);
		$("#nombre").val(data.est_pqr_nombre);
		$("#desc").val(data.est_pqr_descripcion);
	})
}

// Funcion para desactivar un registro
function desactivar(est_pqr_id){
	bootbox.confirm("¿Esta seguro de desactivar el estado de PQR's?",function(result){
		if (result) {
			$.post("../ajax/estadoPqr.php?op=desactivar", {est_pqr_id : est_pqr_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(est_pqr_id){
	bootbox.confirm("¿Esta seguro de activar el estado de PQR's?",function(result){
		if (result) {
			$.post("../ajax/estadoPqr.php?op=activar", {est_pqr_id : est_pqr_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}

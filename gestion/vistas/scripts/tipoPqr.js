...
function listar(){
	...
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":{
			url:'../ajax/tipoPqr.php?op=listar',
			type: "get",
			dataType: "json",
			error: function(e){
...
function guardaryeditar(e){
	...
	$.ajax({
		...
		url: "../ajax/tipoPqr.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(tip_pqr_id){
	$.post("../ajax/tipoPqr.php?op=mostrar",{tip_pqr_id : tip_pqr_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#tip_pqr_id").val(data.tip_pqr_id);
		$("#nombre").val(data.tip_pqr_nombre);
		$("#desc").val(data.tip_pqr_descripcion);
	})
}

// Funcion para desactivar un registro
function desactivar(tip_pqr_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de PQR's?",function(result){
		if (result) {
			$.post("../ajax/tipoPqr.php?op=desactivar", {tip_pqr_id : tip_pqr_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(tip_pqr_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de PQR's?",function(result){
		if (result) {
			$.post("../ajax/tipoPqr.php?op=activar", {tip_pqr_id : tip_pqr_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}

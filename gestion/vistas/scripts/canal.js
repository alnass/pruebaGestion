...
function mostrar(can_id){
	$.post("../ajax/canal.php?op=mostrar",{can_id : can_id}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
		// $("#nombredelinput") Corresponde al name de los inputs del form
		$("#can_id").val(data.can_id);
		$("#nombre").val(data.can_nombre);
		$("#desc").val(data.can_descripcion);
	})
}

// Funcion para desactivar un registro
function desactivar(can_id){
	bootbox.confirm("¿Esta seguro de desactivar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/canal.php?op=desactivar", {can_id : can_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
// Funcion para desactivar un registro
function activar(can_id){
	bootbox.confirm("¿Esta seguro de activar el tipo de persona?",function(result){
		if (result) {
			$.post("../ajax/canal.php?op=activar", {can_id : can_id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
